    ...
    return Consumer<AdminStudentProvider>(
      builder: (context, provider, _) {
        ...
        if (provider.errorMessage != null && provider.students.isEmpty) {
          return _ErrorState(message: provider.errorMessage!, onRetry: () => provider.loadStudents());
        }
          ...
          child: Column(
            children: [
              ...
              ListView.separated(
                ...
                separatorBuilder: (_, __) => const Divider(height: 1, color: LmsColors.border),
                itemBuilder: (context, index) {
                  return _StudentRow(student: provider.students[index]);
                },
              ),
              if (pagination != null && pagination.totalPages > 1)
                _PaginationBar(
                  ...
                  onPageChange: (newPage) => provider.loadStudents(page: newPage, limit: pagination.limit),
                ),
            ],
          ),
        );
...
class _ListHeaderRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const style = TextStyle(fontSize: 11.5, fontWeight: FontWeight.w700, color: LmsColors.textGrey, letterSpacing: 0.3);
    return Container(
...
class _StudentRow extends StatelessWidget {
  ...
  String get _initial =>
      (student.name?.isNotEmpty == true ? student.name![0] : student.email[0]).toUpperCase();
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final nameBlock = Column(
          ...
          children: [
            Text(
              ...
              style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14.5, color: LmsColors.textDark),
        ...
        final phoneBlock = Text(
          (student.phone != null && student.phone!.isNotEmpty) ? student.phone! : '—',
          style: const TextStyle(color: LmsColors.textDark, fontSize: 13),
        );

        final statusBlock = _StatusChip(status: student.status);

        final courseBlock = course != null
            ? _CourseChip(title: course.title, isPremium: course.isPremium)
            : Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
          decoration: BoxDecoration(color: LmsColors.bg, borderRadius: BorderRadius.circular(20)),
          child: const Text('Not enrolled', style: TextStyle(fontSize: 11.5, color: LmsColors.textGrey)),
        );
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(spacing: 6, runSpacing: 6, children: [statusBlock, courseBlock]),
                  ],
                ),
              ),
            ],
...
class _StatusChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isActive = status.toLowerCase() == 'active' || status.toLowerCase() == 'verified';
    final color = isActive ? LmsColors.success : const Color(0xFFFF9F43);
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(status, style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w700, color: color)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(isPremium ? Icons.workspace_premium_rounded : Icons.school_rounded, size: 13, color: color),
...
class _PaginationBar extends StatelessWidget {
  ...
  const _PaginationBar({required this.pagination, required this.isLoading, required this.onPageChange});

  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 12),
      decoration: const BoxDecoration(border: Border(top: BorderSide(color: LmsColors.border))),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          IconButton(
            icon: const Icon(Icons.chevron_left_rounded),
            onPressed: isLoading || pagination.page <= 1 ? null : () => onPageChange(pagination.page - 1),
          ),
          Text(
            'Page ${pagination.page} of ${pagination.totalPages}',
            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: LmsColors.textDark),
          ),
          IconButton(
            icon: const Icon(Icons.chevron_right_rounded),
            onPressed: isLoading || pagination.page >= pagination.totalPages ? null : () => onPageChange(pagination.page + 1),
          ),
        ],
      ),
    );
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: LmsColors.bg, shape: BoxShape.circle),
              child: const Icon(Icons.people_outline_rounded, size: 32, color: LmsColors.textGrey),
            ),
            const SizedBox(height: 14),
            const Text('No students found', style: TextStyle(color: LmsColors.textGrey, fontSize: 14.5, fontWeight: FontWeight.w600)),
          ],
        ),
      ),
    );
    ...
    return Padding(
      ...
      child: Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.error_outline_rounded, size: 36, color: LmsColors.error),
            const SizedBox(height: 12),
            Text(message, textAlign: TextAlign.center, style: const TextStyle(color: LmsColors.textDark)),
            const SizedBox(height: 14),
            ElevatedButton(
              onPressed: onRetry,
              style: ElevatedButton.styleFrom(backgroundColor: LmsColors.primary, foregroundColor: Colors.white),
